...
const MainData = () => {
    ...
    const { products, orders, customers , loading } = useSelector((state) => state.mainData);
    ...
    let totalAmount = orders.reduce((total, order) => total + parseFloat(order.totalPrice.$numberDecimal), 0);
        ...
        datasets: [
            {
                ...
                data: months.map((m, i) => orders.filter((od) => new Date(od.createdAt).getMonth() === i && new Date(od.createdAt).getFullYear() === date.getFullYear() - 2).reduce((total, od) => total + parseFloat(od.totalPrice), 0)),
            },
            {
                label: `Sales in ${date.getFullYear() - 1}`,
                borderColor: 'orange',
                backgroundColor: 'orange',
                data: months.map((m, i) => orders.filter((od) => new Date(od.createdAt).getMonth() === i && new Date(od.createdAt).getFullYear() === date.getFullYear() - 1).reduce((total, od) => total + parseFloat(od.totalPrice), 0)),
            },
            {
                label: `Sales in ${date.getFullYear()}`,
                borderColor: '#4ade80',
                backgroundColor: '#4ade80',
                data: months.map((m, i) => orders.filter((od) => new Date(od.createdAt).getMonth() === i && new Date(od.createdAt).getFullYear() === date.getFullYear()).reduce((total, od) => total + parseFloat(od.totalPrice), 0)),
            },
        ],
    };
    ...
    const pieState = {
        ...
        datasets: [
            {
                backgroundColor: ['#9333ea', '#facc15', '#4ade80', '#f87171', '#a3a3a3'],
                hoverBackgroundColor: ['#a855f7', '#fde047', '#86efac', '#fca5a5', '#d4d4d4'],
